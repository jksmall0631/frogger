...
var DeathAudio = require('./deathaudio.js');
...
var deathAudio = new DeathAudio(deathMusic);
var death = false;
var start = false;
...
function logCollusion(){
  ...
  if (frog.x < log.x + log.width - 50 &&
   frog.x + frog.width > log.x + 50 &&
...
function lilyPadCollusion(){
  allLilyPads.forEach(function(lilypad, i){
    var currentFrog = new Frog (lilypad.x-15, lilypad.y, 45, 50);
  if (frog.x < lilypad.x + lilypad.width &&
   ...
   frog.height + frog.y > lilypad.y) {
     ...
     frog.x = (width -50)/2;
     frog.y = height -100;
     currentFrog.drawFrog(ctx, frogImg);
...
function playMain(mainAudio,introAudio) {
  froggerMusic.play();
  froggerIntro.stop();
}
