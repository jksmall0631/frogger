...
function turtleCollusion(){
  ...
  if (frog.x < allTurtles[i].x + allTurtles[i].width &&
   frog.x + frog.width > allTurtles[i].x &&
   frog.y < allTurtles[i].y + allTurtles[i].height &&
   frog.height + frog.y > allTurtles[i].y) {
     (frog.x = turtle.x + turtle.height/500);
     (frog.y = turtle.y + turtle.width/500);
     }
});
}
